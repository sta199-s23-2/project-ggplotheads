...
What is the relationship between age and quantity of medals won in the Winter Olympics across different sports? Demonstrate using a visualization.
...
Question: Are Nordic or U.S. skiers winning more medals in skiing events in the Winter Olympics?

Create a linear model to describe the relationship between the age of a Nordic Olympian and the number of medals won in a skiing event.

```{r}
#| label: wrangle-the-data

Nordic <- WinterOlympic |>
  filter(Country == "Norway" | 
         Country == "Sweden" | 
         Country == "Denmark" | 
         Country == "Finland")
...
```{r}
#| label: Create-linear-regression-Nordic
...
Create a linear model to describe the relationship between the age of a U.S. Olympian and the number of medals won in a skiing event.

```{r}
#| label: wrangle-data-for-US
US <- WinterOlympic |>
  ...
  AgeOlympian <- US |>
  filter(!is.na(`Age of Athlete`)) |>
  mutate(Age_Classification = if_else(Age > 30, "Old Olympian", "Young Olympian")) |>
...
```{r}
#| label: count-US-medal
WinterOlympic |>
  count(Medal)
```

```{r}
#| label: Create-linear-regression-US
...
```{r}
WinterOlympic |>
   filter(Country %in% c("United States", "Russia", "China", "Norway", "Netherlands", "Finland"),
          Sport == "Speedskating") |>
...
For every 1 year increase, we estimate a mean change of 0.009 years in age for a Winter Olympic gold medal winner. This shows that age for the top athletes remains pretty consistent over time, meaning the winningest athletes are not getting older or younger, they are mostly the same age.
