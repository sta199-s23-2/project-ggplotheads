---
title: "Winter Olympics Data Analysis"
subtitle: "Report"
format: html
editor: visual
---
...
This p-value is 0.71 which means that given the null hypothesis is true (the mean medal rank for men and women in the United States is equal), there is a 0.71 chance that the alternative hypothesis is true. Therefore, given an $\alpha$ = 0.01, this null hypothesis cannot be rejected.

```{r}
#| label: gender_model

gender_model <- linear_reg() |>
 set_engine("lm") |>
 fit(`Medal Rank`~ Gender, data = OlympicGender) |>
  tidy()
...
age_model <- linear_reg() |>
 set_engine("lm") |>
 fit(`Medal Rank`~ `Age of Athlete`, data = OlympicGender) |>
 tidy()

age_model

gender_age_model <- linear_reg() |>
 set_engine("lm") |>
 fit(`Medal Rank`~ `Age of Athlete` + Gender, data = OlympicGender) |>
  tidy()

gender_age_model

gender_age_model_int <- linear_reg() |>
 set_engine("lm") |>
 fit(`Medal Rank`~ `Age of Athlete` * Gender, data = OlympicGender) |>
  tidy()

gender_age_model_int


gender_model_AIC <- linear_reg() |>
 set_engine("lm") |>
 fit(`Medal Rank`~ Gender, data = OlympicGender) |>
 glance() |>
 pull(AIC)

age_model_AIC <- linear_reg() |>
 set_engine("lm") |>
 fit(`Medal Rank`~ `Age of Athlete`, data = OlympicGender) |>
 glance() |>
 pull(AIC)

gender_age_model_AIC <- linear_reg() |>
 set_engine("lm") |>
 fit(`Medal Rank`~ `Age of Athlete` + Gender, data = OlympicGender) |>
 glance() |>
 pull(AIC)
...
gender_age_model_int_AIC <- linear_reg() |>
 set_engine("lm") |>
 fit(`Medal Rank`~ `Age of Athlete` * Gender, data = OlympicGender) |>
 glance() |>
 pull(AIC)
...
gender_model_AIC
age_model_AIC
gender_age_model_AIC
gender_age_model_int_AIC
```

Medal rank and gender: $\widehat{Medal\_Rank} = 1.929 + 0.02786*GenderWomen$
...
WinterOlympic |>
   ...
   labs(title = "Olympic speedskating (ice event) success by country \n throughout years")
```
...
WinterOlympic |>
   ...
   ggplot(aes(x = Year, fill = Country)) +
   geom_density(alpha = 0.4) +
   labs(title = "Olympic alpine skiing (skiing/snowboarding event) success by country \n throughout years")
```
...
WinterOlympic |>
   ...
   ggplot(aes(x = Year, fill = Country)) +
   geom_density(alpha = 0.4) +
   labs(title = "Olympic cross-country skiing success by country \n throughout years")
```

This density graph shows that the United States has become more successful in speedskating and alpine skiing in recent years. However, in the early Olympic years, Finland had the most success in speedskating and since dropped off. Olympians from Norway and Sweden have had the least success in speedskaing in recent years. Additionally, Scandinavian countries have recently statrted seeing greater success in alpine skiing, but the United States was more succcessful in the past. Yet, when looking at a Nordic event like cross-country skiing it is seen that the United States does not have any documented medals received while Scandinavian countries appear to outperform in this event as compared to the other two plotted.
...
Work Cited 1)Griffin, Randi H. "Olympic History: Scraping Data from Sports-Reference.com." Randigriffin.com, 27 May 2018, http://www.randigriffin.com/2018/05/27/olympic-history-1-web-scraping.html.
